...
Proto.Menu = Class.create({
	initialize: function() {
		...
		this.options = Object.extend({
			...
			fade: false,
			zIndex: 100,
			beforeShow: function() { var elements = document.getElementsByName("contextMenu"); for(i=0; i<elements.length; i++) { elements[i].style.visibility="hidden"; }},// var elements = document.getElementsByName("contextMenu"); for(i=0; i<=elements.length; i++) { alert(i); elements[i].hide(); }},
			beforeHide: e,
			beforeSelect: e
		}, arguments[0] || { });
		...
		this.container = new Element('div', {className: this.options.className, style: 'display:none', name: 'contextMenu'});
